 ...
 * USA.
 */
package com.function.plantuml_server;
...
public class DiagramResponse {
     ...
     */
    public DiagramResponse(Builder res, FileFormat fmt, HttpRequestMessage<Optional<String>> req) {
        response = res;
        format = fmt;
        request = req;
    }
    ...
    public void sendDiagram(String uml, int idx) throws IOException {
        ...
        response.header("Content-Type", getContentType());

        if (idx < 0) {
            response.status(HttpStatus.BAD_REQUEST).body(String.format("Invalid diagram index: {0}", idx));
            return;
        }
        final SourceStringReader reader = getSourceStringReader(uml);
        if (reader == null) {
            response.status(HttpStatus.BAD_REQUEST).body("No UML diagram found");
            return;
        }
        ...
        if (format == FileFormat.BASE64) {
            ...
            response.body(encodedBytes.getBytes());
            return;
        }
        ...
        diagram.exportDiagram(os, blockSelection.systemIdx, new FileFormatOption(format));
    ...
    private boolean notModified(BlockUml blockUml) {
        ...
        final long ifModifiedSince = toDateTime(request.getHeaders().get("If-Modified-Since"));
    ...
    public void sendCheck(String uml) throws IOException {
        ...
        response.body(desc.getDescription());
    }
    ...
    private void addHeaderForCache(BlockUml blockUml) {
        ...
        response.header("Expires", toDateString(today + 1000L * maxAge));
        response.header("Date", toDateString(today));

        response.header("Last-Modified", toDateString(blockUml.lastModified()));
        response.header("Cache-Control", "public, max-age=" + maxAge);
        // response.addHeader("Cache-Control", "max-age=864000");
        response.header("Etag", "\"" + blockUml.etag() + "\"");
        final Diagram diagram = blockUml.getDiagram();
        response.header("X-PlantUML-Diagram-Description", diagram.getDescription().getDescription());
        if (diagram instanceof PSystemError) {
            final PSystemError error = (PSystemError) diagram;
            for (ErrorUml err : error.getErrorsUml()) {
                response.header("X-PlantUML-Diagram-Error", err.getError());
                response.header("X-PlantUML-Diagram-Error-Line", "" + err.getLineLocation().getPosition());
            }
        }
        addHeaders();
    }
    ...
    private void addHeaders() {
        ...
        response.header("X-Patreon", "Support us on https://plantuml.com/patreon");
        response.header("X-Donate", "https://plantuml.com/paypal");
    }
    ...
    private long toDateTime(String datestr) {
        try {
            return new SimpleDateFormat().parse(datestr).getTime();
        } catch (Exception e) {
            // ignore
        }
        return -1;
